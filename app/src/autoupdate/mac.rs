...
pub(super) fn relaunch() -> Result<()> {
    ...
    // We need to make sure that the current Swarf process is no longer running
...
/// Determines if the user needs authorization in order to update Swarf.
...
/// * Swarf can create files in the directory
/// * Swarf can modify the permissions of created files
...
/// team identifier matches Swarf's team identifier.
...
async fn apply_update(channel: Channel, version_info: &VersionInfo, update_id: &str) -> Result<()> {
    ...
    if async_fs::metadata(executable_temp_file.as_path())
        ...
        .is_ok()
    {
        // If we performed this process already but didn't relaunch Swarf, the old executable will
...
async fn mount_dmg(dmg_dir: &Path, update_id: &str) -> Result<PathBuf> {
    ...
    // -nobrowse: Do not show the Swarf DMG in Finder or similar apps.
    // -noautoopen: Do not open the Swarf DMG in Finder.
