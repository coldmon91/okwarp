//! Generates a JSON Schema file describing Swarf's user-facing settings.
//!
//! Usage:
//! ```
...
fn main() {
    ...
    root.insert(
        "title".to_string(),
        Value::String("Swarf Settings".to_string()),
    );
    root.insert(
        "description".to_string(),
        Value::String(format!(
            "JSON Schema for Swarf settings ({channel} channel, {entry_count} settings)"
        )),
    );
