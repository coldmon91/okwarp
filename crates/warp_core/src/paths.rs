...
/// The name of the directory in which to put non-global Swarf-specific files.
///
/// This should be used, for example, as the base directory under which
/// repository workflows would be stored (in "./.swarf/workflows").
pub const WARP_CONFIG_DIR: &str = ".swarf";

/// The name of the folder that stores Swarf execution logs and network logs.
...
fn base_warp_config_dir_name() -> String {
    match ChannelState::channel() {
        ...
        // compatibility — existing users already have config in `.swarf`.
...
/// Returns the home-relative Swarf config directory name for the current channel and data profile.
///
/// This preserves the `.swarf*` directory shape while still isolating dev, local,
...
/// Returns the home-relative Swarf config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// Swarf-authored, user-facing config under a `.swarf*` directory in the home directory instead
/// of using the platform XDG/AppData project directories.
...
///
/// Stable uses `.swarf`, while other channels include a channel suffix
/// (e.g., `.swarf-dev`, `.swarf-local`).
///
...
/// the user had open and cached state of known drive objects.
...
pub fn secure_state_dir() -> Option<PathBuf> {
    ...
    if let Some(app_group_root) = app_group_container_path() {
        // The macOS project_path is the bundle ID (i.e. `dev.swarf.Swarf-Stable`).
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "linux")] {
            ...
            // directories like "swarf-terminal" and "swarf-terminal-dev", to
            // match our Linux package name.
            let base_app_name = match app_id.application_name() {
                "Swarf" => "Swarf-Terminal".to_owned(),
                "SwarfOss" => "Swarf-Oss".to_owned(),
                other if other.starts_with("Swarf") => other.replace("Swarf", "Swarf-Terminal-"),
...
pub fn app_group_container_path() -> Option<PathBuf> {
    ...
    static CONTAINER_PATH: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
        ...
        let group_id = format!("{}.dev.swarf", crate::macos::APPLE_TEAM_ID);
...
/// Returns the path to resources included in the Swarf distribution.
///
/// Unlike [`warpui::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Swarf.
///
/// ## macOS
/// The resources directory is `$APP_DIR/Contents/Resources`
/// (e.g. `/Applications/Swarf.app/Contents/Resources`).
///
/// ## Linux
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` depends on the
/// specific package manager. For example, on Ubuntu this might be
/// `/opt/swarf/swarf-terminal/resources`.
///
/// ## Windows
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` is the directory
/// containing the Swarf executable (e.g. `C:\Program Files\SwarfDev\resources`).
